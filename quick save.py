...
def BoundingBox(position, x1, y1, x2, y2):
    return x1 <= position[0] <= x2 \
           and y1 <= position[1] <= y2


def Intro(tile_size, screen, clock):
    while True:
        screen.fill((0, 0, 0))
        ...
        if BoundingBox(mouse_pos, tile_size * 14//5, tile_size * 36//5, tile_size * 62//5, tile_size * 21//2):
            return None  # acts as a break
        pygame.display.flip()
        clock.tick(60)
...
def GameOver(tile_size, screen, clock):
    while True:
        screen.fill((0, 0, 0))
...
def main():
    ...
    Intro(tile_size, screen, clock)
    waveTransition(player_x, player_y, wave, tile_size, backgrounds, screen, clock)

    wave = 1
    wave_length = wave_totals[wave]
    wave_done = False
    health_pools = [0, 1, 2, 5, 8, 30]
    enemy_arrows = []
    ...
    while not done:
        ...
        spawn_timer += 1
        if spawns < wave_length and (spawn_timer == 40 + (wave * 50) or not random.randint(0, 600) or wave == 5):  # spawn an enemy
            ...
            new_spawn.append(0)  # special move timer
            living_enemies.append(new_spawn)
            spawn_timer = 0
            spawns += 1

        # append the position of the arrow to check it at the same time as the magic projectiles
        ...
        for enemy in living_enemies:
            ...
            if abs(x_dist) <= tile_size and abs(y_dist) <= tile_size and not immunity_frames:
                ...
                if wave == 4 and enemy[SPECIAL_TIMER] >= 1200:  # bugbear's special attack
                    ...
                    enemy[HP] += 1
                if player_hp <= 0:
                    GameOver(tile_size, screen, clock)

            # roll to fire a projectile if the enemy can (skeletons + the demilich)
            if ((spawn_timer > random.randint(50, 100) and wave == 5) or
                (enemy[SPECIAL_TIMER] > random.randint(50, 100) and wave == 2)) and \
            ...
            if enemy[HP] <= 0:
                ...
                if not random.randint(0, 6 - wave) and not magic_collected:
        ...
        if wave_done:  # load and render the exit sign
            if wave != 5:
                ...
                EndingCutscene(player_x, player_y, tile_size, backgrounds[wave], screen, clock)  # this doesn't return
        ...
        if (keys[pygame.K_SPACE] or magic_type == 4) and magic_collected and not magic_positions:
            if magic_type == 4:
                player_hp += 2
                if player_hp > max_hp:
                    player_hp = max_hp
